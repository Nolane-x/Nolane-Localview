...
impl ServiceGraph {
    pub fn add_service(&mut self, service: LocalService) {
        self.services.insert(service.id.clone(), service);
    }

    pub fn add_edge(&mut self, edge: ServiceEdge) -> bool {
        if !self.services.contains_key(&edge.from) || !self.services.contains_key(&edge.to) {
            return false;
        }
...
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct FullStackErrorChain {
...
pub struct AsyncFlowGraph {
    pub nodes: BTreeMap<String, AsyncFlowNode>,
    pub edges: Vec<AsyncFlowEdge>,
}
...
pub fn safe_checkpoint(checkpoint: &FullStackCheckpoint) -> bool {
    checkpoint.database_mutations_blocked
        && checkpoint.pending_async_flows.is_empty()
        && !checkpoint.revision.is_empty()
}
...
mod tests {
    ...
    fn schema_drift_marks_new_required_field_as_breaking() {
        let before = SchemaShape {
            name: "User".into(),
            fields: BTreeMap::from([("id".into(), "string".into())]),
            required: BTreeSet::from(["id".into()]),
            examples: BTreeMap::new(),
        };
        let after = SchemaShape {
            name: "User".into(),
            fields: BTreeMap::from([
                ("id".into(), "string".into()),
                ("email".into(), "string".into()),
            ]),
            required: BTreeSet::from(["id".into(), "email".into()]),
            examples: BTreeMap::new(),
        };
    ...
    fn service_graph_follows_local_dependencies() {
        ...
        for (id, kind) in [
            ("ui", ServiceKind::Frontend),
            ("api", ServiceKind::Api),
            ("db", ServiceKind::Database),
        ] {
            graph.add_service(LocalService {
                id: id.into(),
                name: id.into(),
                kind,
                address: None,
