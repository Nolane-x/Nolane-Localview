...
impl Default for ProjectPreferences {
    fn default() -> Self {
        Self {
            auto_open: AutoOpenPreference::FirstSession,
            remember_window: true,
            xray_default: false,
            preferred_viewport: None,
            notifications: NotificationPreference::ImportantOnly,
        }
    }
}
...
pub fn open_decision(context: &AutoOpenContext, preference: AutoOpenPreference) -> OpenDecision {
    if context.runtime_paused && !context.explicit_open_request {
        return OpenDecision::StayHidden;
    }
    if context.existing_preview_window
        && (context.explicit_open_request || context.user_active_in_localview)
    {
        return OpenDecision::FocusExisting;
    }
    if context.explicit_open_request {
        return OpenDecision::OpenNew;
    }
    match preference {
        AutoOpenPreference::Never => OpenDecision::StayHidden,
        AutoOpenPreference::FirstSession
            if context.first_session_for_project && context.is_frontend =>
        {
            OpenDecision::OpenNew
        }
        AutoOpenPreference::FrontendOnly
            if context.is_frontend && !context.user_active_in_localview =>
        {
            OpenDecision::OpenNew
        }
...
pub fn should_notify(
    ...
    if app_focused && importance < EventImportance::ActionRequired {
        return false;
    }
    match preference {
...
pub fn tray_actions(paused: bool, has_session: bool) -> Vec<TrayAction> {
    ...
    if has_session {
        actions.push(TrayAction::OpenLastSession);
    }
    actions.push(if paused {
        TrayAction::ResumeDetection
    } else {
        TrayAction::PauseDetection
    });
...
impl OnboardingState {
    pub fn next_hint(&self) -> Option<&'static str> {
        if !self.daemon_ready {
            Some("Start the LocalView runtime")
        } else if !self.frontend_detected {
            Some("Run a local frontend dev server")
        } else if !self.preview_opened {
            Some("Open the detected preview")
        } else if !self.agent_connected {
            Some("Optional: connect CLI or MCP")
        } else {
            None
        }
    }
...
mod tests {
    ...
    fn existing_window_is_focused_instead_of_spawned() {
        let context = AutoOpenContext {
            is_frontend: true,
            first_session_for_project: false,
            user_active_in_localview: true,
            existing_preview_window: true,
            runtime_paused: false,
            explicit_open_request: true,
        };
        assert_eq!(
            open_decision(&context, AutoOpenPreference::Always),
