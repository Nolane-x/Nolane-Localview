...
mod tests {
  ...
  const reactComponentName = (type) => {
    ...
    for (const candidate of candidates) {
      if (!candidate || (typeof candidate !== 'function' && typeof candidate !== 'object')) continue;
      const name = boundedUtf8String(candidate.displayName || candidate.name, MAX_REACT_COMPONENT_BYTES);
      if (name) return name;
    }
    return null;
  };
  ...
  const reactSourceHint = (el, ownershipBudget) => {
    ...
    for (let depth = 0; cursor && depth < MAX_REACT_FIBER_DEPTH; depth += 1, cursor = cursor.return) {
      if (typeof cursor !== 'object') break;
      const component = reactComponentName(cursor.type);
      const debugSource = cursor._debugSource;
      if (!component || !debugSource || typeof debugSource !== 'object') continue;

      const file = boundedUtf8String(debugSource.fileName, MAX_REACT_SOURCE_FILE_BYTES);
      const line = Number(debugSource.lineNumber);
      const column = debugSource.columnNumber == null ? null : Number(debugSource.columnNumber);
      if (!file
          || !Number.isInteger(line)
          || line < 1
          || line > MAX_REACT_SOURCE_LINE
          || (column !== null
            && (!Number.isInteger(column)
              || column < 1
              || column > MAX_REACT_SOURCE_COLUMN))) {
        continue;
      }
