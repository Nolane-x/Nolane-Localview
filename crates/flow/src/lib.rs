...
impl InteractionGraph {
    pub fn record(&mut self, state: &str, transition: Transition) {
        let e = self.edges.entry(state.into()).or_default();
        if !e.iter().any(|x| {
            x.action == transition.action && x.resulting_state == transition.resulting_state
        }) {
            e.push(transition);
        }
    }
    pub fn shortest_path(&self, from: &str, to: &str) -> Option<Vec<Transition>> {
        ...
        while let Some((state, path)) = q.pop_front() {
            if state == to {
                return Some(path);
            }
            ...
            for edge in self.edges.get(&state).into_iter().flatten() {
                ...
                q.push_back((edge.resulting_state.clone(), next));
            }
        }
        None
    }
...
mod tests {
    ...
    fn finds_flow() {
        ...
        assert_eq!(g.shortest_path("/", "/login").unwrap().len(), 1);
    }
}
