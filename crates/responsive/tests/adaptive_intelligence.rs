use localview_responsive::{
    DEFAULT_ADAPTIVE_PROBE_CAP, LayoutProbe, ObservedTransitionResolution, ResponsiveDetectorState,
    ResponsiveIssueClass, ResponsiveIssueKind, ResponsiveNodeObservation, ResponsiveObservation,
    ResponsiveProbeEvaluation, ResponsiveProbeSample, ResponsiveRect, Viewport,
    analyze_responsive_series, bounded_adaptive_sweep, deduplicate_responsive_issues,
    discover_breakpoint, evaluate_responsive_observation, resolve_observed_transition,
};
...
fn observation(
    width: u32,
    version: u64,
    nodes: Vec<ResponsiveNodeObservation>,
...
fn adaptive_planner_rejects_invalid_width_arithmetic_and_caps() {
    ...
    assert!(bounded_adaptive_sweep(320, 1440, &[], DEFAULT_ADAPTIVE_PROBE_CAP + 1).is_err());
}
...
fn observed_transition_requires_one_monotonic_bracket() {
    let resolved = resolve_observed_transition(
        &[
            ResponsiveProbeSample {
                width: 700,
                state: ResponsiveDetectorState::Fail,
            },
            ResponsiveProbeSample {
                width: 716,
                state: ResponsiveDetectorState::Fail,
            ...
            ResponsiveProbeSample {
                width: 728,
                state: ResponsiveDetectorState::Pass,
            },
        ],
        16,
    ...
    match resolved {
        ObservedTransitionResolution::Resolved {
            ...
            assert_ne!(
                ...
                "must remain a bracket, not an exact CSS breakpoint"
            );
        }
...
fn no_transition_is_explicit() {
    ...
    assert!(matches!(
        result,
        ObservedTransitionResolution::NoTransition { .. }
    ));
}
...
async fn binary_discovery_is_bounded_for_the_supported_width_domain() {
    let probe = CountingMonotonicProbe {
...
fn breakpoint_local_regression_and_nearby_instability_are_explicit() {
    ...
    assert!(
        issues
            .iter()
            .any(|issue| issue.kind == ResponsiveIssueKind::BreakpointLocalRegression)
    );
    assert!(
        issues
            .iter()
            .any(|issue| issue.kind == ResponsiveIssueKind::NearbyWidthInstability)
    );
}
