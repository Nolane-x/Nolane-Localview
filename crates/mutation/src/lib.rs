...
pub fn evaluate_mutation(
    case: &MutationCase,
    triggered_detectors: BTreeSet<String>,
    evidence_ids: Vec<String>,
) -> MutationOutcome {
    if !case.safe_to_run {
        return MutationOutcome {
            mutation_id: case.id,
            verdict: MutationVerdict::SkippedUnsafe,
            triggered_detectors,
            evidence_ids,
        };
    }
...
pub fn quality_map(cases: &[MutationCase], outcomes: &[MutationOutcome]) -> VerificationQualityMap {
    let outcome_by_id = outcomes
        .iter()
        .map(|outcome| (outcome.mutation_id, outcome))
        .collect::<BTreeMap<_, _>>();
    let valid = cases
        .iter()
        .filter(|case| case.safe_to_run && !case.expected_detectors.is_empty())
        .collect::<Vec<_>>();
    let killed_count = valid
        .iter()
        .filter(|case| {
            outcome_by_id
                .get(&case.id)
                .is_some_and(|outcome| outcome.verdict == MutationVerdict::Killed)
        })
        .count();
    let total_weight = valid
        .iter()
        .map(|case| case.relevance.max(0.0))
        .sum::<f32>();
    let killed_weight = valid
        .iter()
        .filter(|case| {
            outcome_by_id
                .get(&case.id)
    ...
    for class in [
        MutationClass::Layout,
        MutationClass::Accessibility,
        MutationClass::Behavior,
        MutationClass::Failure,
        MutationClass::Content,
        MutationClass::Visual,
    ] {
        let class_cases = valid
            .iter()
            .filter(|case| case.operator.class() == class)
            .collect::<Vec<_>>();
        if class_cases.is_empty() {
            continue;
        }
        let killed = class_cases
            .iter()
            .filter(|case| {
                outcome_by_id
                    .get(&case.id)
    ...
    VerificationQualityMap {
        overall_kill_rate: if valid.is_empty() {
            1.0
        } else {
            killed_count as f32 / valid.len() as f32
        },
        weighted_kill_rate: if total_weight <= f32::EPSILON {
            1.0
        } else {
            killed_weight / total_weight
        },
        by_class,
        surviving_mutations,
...
mod tests {
    ...
    fn mutation_is_killed_when_expected_detector_fires() {
        let case = MutationCase {
            id: Uuid::new_v4(),
            target: "hero".into(),
            operator: MutationOperator::Shift { dx: 20.0, dy: 0.0 },
            expected_detectors: BTreeSet::from(["layout-diff".into()]),
            safe_to_run: true,
            relevance: 1.0,
        };
        let outcome = evaluate_mutation(
            &case,
            BTreeSet::from(["layout-diff".into()]),
            vec!["ev".into()],
        );
    ...
    fn surviving_relevant_mutation_reduces_quality_score() {
        let case = MutationCase {
            id: Uuid::new_v4(),
            target: "button".into(),
            operator: MutationOperator::DisableHandler,
            expected_detectors: BTreeSet::from(["dead-click".into()]),
            safe_to_run: true,
            relevance: 1.0,
        };
