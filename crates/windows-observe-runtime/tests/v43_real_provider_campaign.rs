...
mod windows_l7_real_provider_campaign {
    ...
    fn write_json(path: &Path, value: &impl serde::Serialize) {
        let bytes = serde_json::to_vec_pretty(value).expect("serialize L7 artifact");
        fs::write(path, bytes).expect("persist L7 artifact");
    }
    ...
    fn run_w01(
        ...
        manager
            .release(session_id)
            .await_blocking();
        assert!(manager.status(session_id).await_blocking().is_none());
        seed.shutdown();
        record
    }
    ...
    fn run_w02(
        ...
        manager.release(session_id).await_blocking();
    ...
    fn run_w06(
        ...
        manager_a.release(session_id).await_blocking();
        assert!(manager_a.status(session_id).await_blocking().is_none());
        assert!(bridge.observation_status(session_id).await_blocking().is_none());
        drop(manager_a);
        ...
        manager_b.release(session_id).await_blocking();
        let cleanup_to_baseline = manager_b.status(session_id).await_blocking().is_none()
            && bridge.observation_status(session_id).await_blocking().is_none();
    ...
    trait BlockOnResult<T> {
        fn await_blocking(self) -> T;
    }

    impl<F, T, E> BlockOnResult<Result<T, E>> for F
    where
        F: std::future::Future<Output = Result<T, E>>,
        E: std::fmt::Debug,
    {
        fn await_blocking(self) -> Result<T, E> {
            tokio::runtime::Handle::current().block_on(self)
        }
    }

    impl<F, T> BlockOnResult<T> for F
    where
    ...
    async fn prospective_l7_campaign_binds_w01_w02_w06_to_exact_candidate() {
        ...
        write_json(&artifact_dir.join("environment-manifest.json"), &environment);
        let environment_digest = canonical_digest(&environment).expect("digest environment manifest");

        let seed_catalog_digest = canonical_digest(&json!({
            "candidate_sha": required_env("LOCALVIEW_CANDIDATE_SHA"),
            "environment_digest": environment_digest.0,
            "seed_executable_digest": seed_digest,
        ...
        let preregistration = LabPreregistration {
            revision_context: LabRevisionContext {
                ...
                random_source_profile: "deterministic-hosted-seed-protocol".into(),
        ...
        let prepared = preregistration.prepare().expect("prepare L7 preregistration");
        ...
        let records = vec![
            run_w01(&seed_digest, &environment_digest_text, 101),
            run_w02(&seed_digest, &environment_digest_text, 102),
            run_w06(&seed_digest, &environment_digest_text, 103),
        ];
        ...
        assert_eq!(campaign_evidence, ResultEvidence::RealProviderIntegrationPass);
        let completed = run
        ...
        write_json(&artifact_dir.join("LAB-RESULT.json"), &completed);
    }
}
