...
fn selection() -> UserSelectedWindowTarget {
    UserSelectedWindowTarget {
        native_window_handle: 0x8202,
        expected_process_id: 82,
        selection_nonce: Uuid::from_u128(0x8203),
    }
}

fn path() -> PathBuf {
    std::env::temp_dir().join(format!(
        "localview-v43-attachment-recovery-plan-clean-{}.jsonl",
        Uuid::new_v4()
    ))
}
...
#[tokio::test]
async fn recovery_plan_is_exact_attachment_bound_ordered_classified_and_side_effect_free() {
    ...
    let admission = journal
        .record_dispatch_prepared(
            prepared.transport_action_id,
            DispatchPreparationReceipt {
                receipt_ref: "prepared:recovery-plan-clean".into(),
    ...
    let wrong_provider = envelope(
        "wrong-provider",
        session(),
        ProviderIncarnationRef::from("provider:windows-uia:different"),
        provider.target.clone(),
    );
    journal
        .record_intent_admitted(wrong_provider.clone())
        .await
        .unwrap();

    let wrong_session = envelope(
        "wrong-session",
        Uuid::from_u128(0x82ff),
        provider.provider.clone(),
        provider.target.clone(),
    );
    journal
        .record_intent_admitted(wrong_session.clone())
        .await
        .unwrap();

    let bindings_before = journal.recovery_binding_inventory().await;
    ...
    assert_eq!(
        plan.entries[0].expected_postcondition_contract_refs,
        vec!["postcondition:not-dispatched"]
    );
    ...
    for mismatched in [&wrong_target, &wrong_provider, &wrong_session] {
        assert!(
            plan.entries
                .iter()
                .all(|entry| entry.action_id != mismatched.transport_action_id),
            "durable debt from a mismatched session/provider/target lineage must be excluded"
        );
    }
    assert_eq!(journal.recovery_binding_inventory().await, bindings_before);
    assert_eq!(
        provider.snapshot_calls(),
        1,
        "planning may inspect the immutable attached snapshot but must not capture provider state again"
    );
