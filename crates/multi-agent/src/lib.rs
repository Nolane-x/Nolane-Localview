...
impl LeaseTable {
    pub fn acquire(
        ...
        let proposed = AgentLease {
            ...
            resources,
            acquired_at: now,
            expires_at: now + ttl.max(Duration::seconds(1)),
        };
        let conflicts = self
            .leases
            .values()
            .filter(|existing| existing.session_id == session_id)
            .filter_map(|existing| conflict(existing, &proposed))
            .collect::<Vec<_>>();
    ...
    pub fn reap(&mut self, now: DateTime<Utc>) {
    ...
    pub fn active_for_session(
        ...
        self.leases
            .values()
            .filter(|lease| lease.session_id == session_id && lease.active_at(now))
            .collect()
    }
}
...
fn conflict(left: &AgentLease, right: &AgentLease) -> Option<Conflict> {
    ...
    let mutation_conflict = left.isolation >= IsolationLevel::IsolatedMutation
        || right.isolation >= IsolationLevel::IsolatedMutation;
    if !mutation_conflict {
        return None;
    }
    Some(Conflict {
        left_lease: left.id,
        right_lease: right.id,
        resources: overlap,
        reason: "overlapping mutable resource lease".into(),
    })
}
...
pub fn detect_journal_conflicts(entries: &[JournalEntry]) -> Vec<JournalConflict> {
    ...
    for (index, left) in entries.iter().enumerate() {
        for right in entries.iter().skip(index + 1) {
            if left.agent_id == right.agent_id || left.session_id != right.session_id {
                continue;
            }
            let Some(target) = left
                .target
                .as_ref()
                .filter(|target| right.target.as_ref() == Some(*target))
            else {
                continue;
            };
            let both_mutate = matches!(
                left.intent,
                ActionIntent::MutateSource | ActionIntent::ApplyCandidate
            ) && matches!(
                right.intent,
                ActionIntent::MutateSource | ActionIntent::ApplyCandidate
            );
            if both_mutate && left.base_revision == right.base_revision {
                conflicts.push(JournalConflict {
                    left_entry: left.id,
                    right_entry: right.id,
                    target: target.clone(),
                    reason: "concurrent mutations share the same base revision".into(),
                });
            }
        }
    }
    ...
    fn shared_perception_does_not_block_another_reader() {
        ...
        table
            .acquire(
                Uuid::new_v4(),
                session,
                IsolationLevel::SharedPerception,
                BTreeSet::from(["hero".into()]),
                Duration::seconds(30),
                now,
            )
            .expect("first lease");
    ...
    fn exclusive_mutation_blocks_overlapping_mutation() {
        ...
        table
            .acquire(
                Uuid::new_v4(),
                session,
                IsolationLevel::ExclusiveMutation,
                BTreeSet::from(["src/App.tsx".into()]),
                Duration::seconds(30),
                now,
            )
            .expect("first lease");
        assert!(
            table
                .acquire(
                    Uuid::new_v4(),
                    session,
                    IsolationLevel::IsolatedMutation,
                    BTreeSet::from(["src/App.tsx".into()]),
                    Duration::seconds(30),
                    now
                )
