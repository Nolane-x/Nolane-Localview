...
pub fn can_restore(checkpoint: &RuntimeCheckpoint, request: &RestoreRequest) -> RestoreDecision {
    ...
    if !checkpoint.restorable {
        reasons.push("checkpoint was recorded as observation-only".into());
    }
    if checkpoint.environment_hash != request.current_environment_hash {
        reasons.push("environment hash differs".into());
    }
    if checkpoint.revision != request.current_revision && !request.allow_revision_mismatch {
        reasons.push("source revision differs".into());
    }
    if reasons.is_empty() {
        RestoreDecision::Allowed
    } else {
        RestoreDecision::Denied { reasons }
    }
}
...
impl CheckpointStore {
    pub fn new(capacity_per_session: usize) -> Self {
        Self {
            capacity_per_session: capacity_per_session.max(2),
    ...
    pub fn push(&mut self, checkpoint: RuntimeCheckpoint) {
        ...
        while queue.len() > self.capacity_per_session {
            queue.pop_front();
        }
    }

    pub fn latest(&self, session_id: SessionId) -> Option<&RuntimeCheckpoint> {
        self.by_session
            .get(&session_id)
...
impl PerceptualCache {
    pub fn put(&mut self, entry: PerceptualCacheEntry) {
        self.entries.insert(entry.key.stable_key(), entry);
    }

    pub fn get(&self, key: &CacheKey) -> Option<&PerceptualCacheEntry> {
    ...
    pub fn invalidate_changed_sources(&mut self, changed_files: &BTreeSet<String>) -> Vec<String> {
        let invalid = self
            .entries
            .iter()
            .filter(|(_, entry)| !entry.source_dependencies.is_disjoint(changed_files))
            .map(|(key, _)| key.clone())
            .collect::<Vec<_>>();
        for key in &invalid {
    ...
    pub fn invalidate_revision(&mut self, revision: &str) -> usize {
        ...
        self.entries
            .retain(|_, entry| entry.key.revision == revision);
        before.saturating_sub(self.entries.len())
    }

    pub fn release_session(&mut self, session_id: SessionId) {
        self.entries
            .retain(|_, entry| entry.key.session_id != session_id);
    }
}
...
mod tests {
    ...
    fn restore_rejects_environment_drift() {
        ...
        let decision = can_restore(
            &checkpoint,
            &RestoreRequest {
                checkpoint_id: checkpoint.id,
                current_revision: "a".into(),
                current_environment_hash: "env-b".into(),
    ...
    fn cache_invalidates_only_dependent_regions() {
        ...
        let make = |region: &str, source: &str| PerceptualCacheEntry {
            key: CacheKey {
                session_id: session,
                revision: "a".into(),
            ...
            object_hashes: vec![],
            source_dependencies: BTreeSet::from([source.into()]),
            created_at: Utc::now(),
        };
