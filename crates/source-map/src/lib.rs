...
pub fn parse_stack_locations(stack: &str) -> Vec<SourceLocation> {
    let re =
        Regex::new(r"(?m)(?:\(|\s|^)([^\s()]+\.(?:tsx?|jsx?|vue|svelte)):(\d+):(\d+)\)?").unwrap();
    re.captures_iter(stack)
        .filter_map(|c| {
            Some(SourceLocation {
                file: c.get(1)?.as_str().to_owned(),
...
pub fn rank_hints(
    ...
    for (k, v) in attributes {
        if k == "data-source" || k == "data-component-source" {
            let (mut file, mut line) = (v.as_str(), None);
            if let Some(i) = v.rfind(':') {
                if let Ok(n) = v[i + 1..].parse() {
                    file = &v[..i];
                    line = Some(n);
                }
            }
    ...
    hints.sort_by_key(|h| std::cmp::Reverse(h.confidence));
    hints
}
...
mod tests {
    ...
    fn parses_vite_stack() {
        let x = parse_stack_locations("at save (src/components/Button.tsx:84:12)");
        assert_eq!(x[0].line, 84);
    }
}
