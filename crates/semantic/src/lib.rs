...
pub fn snapshot_diff(before: &PageSnapshot, after: &PageSnapshot) -> StateDiff {
    let old = flatten(&before.root);
    let new = flatten(&after.root);
...
pub fn compact_snapshot(snapshot: &PageSnapshot, max_nodes: usize) -> serde_json::Value {
    ...
    serde_json::json!({
        ...
        "tree": compact(&snapshot.root, &mut remaining),
...
pub fn stable_ref(role: Option<&str>, name: Option<&str>, tag: &str, ancestry: &[&str]) -> ElementRef {
    ...
    for byte in ancestry
        .iter()
        .copied()
        .chain([role.unwrap_or(""), name.unwrap_or(""), tag])
        .flat_map(str::bytes)
    {
        hash ^= byte as u64;
...
mod tests {
    ...
    fn node(r: &str, x: f64) -> SemanticNode {
        SemanticNode {
            reference: r.into(),
    ...
    fn snap(v: u64, x: f64) -> PageSnapshot {
        PageSnapshot {
            version: v,
            route: "/".into(),
            viewport: (1440, 900),
    ...
    #[test]
    fn geometry_change_becomes_layout_delta() {
        let d = snapshot_diff(&snap(1, 0.0), &snap(2, 7.0));
        assert_eq!(d.layout_changes.len(), 1);
        assert!(d.changed_refs.is_empty());
    }
