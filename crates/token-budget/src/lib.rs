...
pub fn serialize_with_budget<T: Serialize>(value: &T, budget: &TokenBudget) -> serde_json::Value {
    ...
    if approximate_tokens(&serialized) <= budget.max_tokens { return full; }
    match budget.detail {
...
fn summary_json(value: serde_json::Value) -> serde_json::Value {
    match value {
        serde_json::Value::Object(map) => serde_json::Value::Object(map.into_iter().filter(|(k, _)| {
            matches!(k.as_str(), "version" | "route" | "changed_refs" | "removed_refs" | "console_delta" | "network_delta" | "layout_changes")
        }).collect()),
        other => other,
    }
}
...
mod tests {
    ...
    #[test]
    fn token_estimate_is_bounded() { assert_eq!(approximate_tokens("12345678"), 2); }
}
