...
use std::{collections::{BTreeMap, VecDeque}, hash::{Hash, Hasher}, path::PathBuf};
use anyhow::Result;
use serde::{Deserialize, Serialize};

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ArtifactMeta { pub id: String, pub kind: String, pub bytes: u64, pub path: String }
...
impl ArtifactStore {
    pub async fn open(root: impl Into<PathBuf>, max_bytes: u64) -> Result<Self> {
        ...
        Ok(Self { root, max_bytes, index: BTreeMap::new(), lru: VecDeque::new(), used: 0 })
    }

    pub async fn put(&mut self, kind: &str, bytes: &[u8]) -> Result<ArtifactMeta> {
        let id = content_id(bytes);
        if let Some(existing) = self.index.get(&id) { return Ok(existing.clone()); }
        let path = self.root.join(&id);
        tokio::fs::write(&path, bytes).await?;
        let meta = ArtifactMeta { id: id.clone(), kind: kind.into(), bytes: bytes.len() as u64, path: path.to_string_lossy().into_owned() };
...
mod tests {
    ...
    async fn dedupes_bytes() {
        let dir = std::env::temp_dir().join(format!("lv-art-{}", std::process::id()));
