...
pub async fn run_w11(
    ...
    let error = worker
        ...
        .expect_err("campaign W11 modal must block before SendInput");
    assert_eq!(
        error,
...
pub async fn run_w12(
    ...
    let stale_error = worker
        ...
        .expect_err("campaign W12 stale authority must be rejected before SendInput");
    ...
    let b_target_incarnation = attachment_b.target_incarnation_ref().clone();
    ...
    authority_b
        .journal
        .abandon_dispatch_execution(authority_b.permit)
        .await
        .expect("campaign W12 fresh B authority is evidence-only and must be abandoned");
    drop(authority_b.journal);
    let _ = std::fs::remove_file(authority_b.journal_path);

    authority_a
        .journal
        .abandon_dispatch_execution(authority_a.permit)
        .await
        .expect("campaign W12 stale A attempt must consume only volatile authority");
    ...
    let ground_truth = json!({
        "process_a": {
            ...
            "target_incarnation": format!("{:?}", a_target_incarnation),
            "terminated_before_dispatch": true
        },
        "process_b": {
            "pid": b_process,
            "window_handle": b_window,
            "target_incarnation": format!("{:?}", b_target_incarnation),
            "effect_count_after_stale_attempt": truth_u64(&after_stale, "effect_count")
        },
    ...
    let record = adapt_real_provider_case(RealProviderCaseInput {
        ...
        provider_evidence_refs: BTreeSet::from([
            ...
            format!("windows-uia:w12:target-a:{:?}", a_target_incarnation),
            format!("windows-uia:w12:target-b:{:?}", b_target_incarnation),
            "windows-uia:w12:typed-blocker:target-reincarnated".into(),
            "windows-uia:w12:replacement-oracle-effect-count:0".into(),
            "windows-uia:w12:fresh-reacquire-required:true".into(),
        ]),
        ground_truth: RealProviderGroundTruth {
            canonical_outcome: "restart-invalidates-stale-authority-before-input".into(),
            digest: canonical_digest(&ground_truth).expect("digest W12 campaign oracle truth"),
        },
        observed_outcome: RealProviderObservedOutcome::Asserted(
            "restart-invalidates-stale-authority-before-input".into(),
        ),
