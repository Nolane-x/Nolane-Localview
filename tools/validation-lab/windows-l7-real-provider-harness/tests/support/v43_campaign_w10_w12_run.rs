...
pub async fn run_exact_campaign() {
    ...
    support::write_value(&artifact_dir.join("environment-manifest-w01-w12.json"), &environment);
    let environment_digest =
        canonical_digest(&environment).expect("digest twelve-seed physical environment manifest");
    ...
    .expect("digest exact twelve-seed catalog");

    let preregistration = LabPreregistration {
        revision_context: LabRevisionContext {
            lab_revision: "lab-v43-windows-l7-r5".into(),
            seed_corpus_revision: "windows-provider-seeds-w01-w12-r5".into(),
    ...
    let prepared = preregistration
        .prepare()
        .expect("prepare twelve-seed physical L7 preregistration");
    let prereg_path = artifact_dir.join("LAB-PREREGISTRATION-W01-W12.json");
    fs::write(&prereg_path, &prepared.canonical_bytes)
        .expect("persist W01-W12 preregistration before campaign start");
    ...
    fs::write(
        artifact_dir.join("LAB-PREREGISTRATION-W01-W12-RECEIPT.json"),
        serde_json::to_vec_pretty(&persisted_receipt)
            .expect("serialize W01-W12 preregistration receipt"),
    )
    .expect("persist W01-W12 preregistration receipt");
    let receipt = validate_persisted_receipt(&prepared, persisted_receipt)
        .expect("validate persisted W01-W12 preregistration receipt");
    ...
    .expect("start typed prospective twelve-seed campaign");
    ...
    fs::write(
        artifact_dir.join("W10-REAL-PROVIDER-RECORD.json"),
        serde_json::to_vec_pretty(&records[9]).expect("serialize W10 real-provider record"),
    )
    .expect("persist W10 real-provider record");
    fs::write(
        artifact_dir.join("W11-REAL-PROVIDER-RECORD.json"),
        serde_json::to_vec_pretty(&records[10]).expect("serialize W11 real-provider record"),
    )
    .expect("persist W11 real-provider record");
    fs::write(
        artifact_dir.join("W12-REAL-PROVIDER-RECORD.json"),
        serde_json::to_vec_pretty(&records[11]).expect("serialize W12 real-provider record"),
    )
    ...
    let evidence = derive_real_provider_campaign_evidence(&records)
        .expect("derive twelve-record provider evidence");
    assert_eq!(evidence, ResultEvidence::RealProviderIntegrationPass);
    let completed = run
        .finalize(evidence, 113)
        .expect("finalize exact twelve-case prospective campaign");
    let rpomr = completed
        .payload
        .metric_snapshot
        .get(LabMetricKind::Rpomr)
        .expect("RPOMR metric");
    assert_eq!((rpomr.numerator, rpomr.denominator), (0, 12));
    assert_eq!(completed.payload.observation_digests.len(), 12);
    assert_eq!(
    ...
    fs::write(
        artifact_dir.join("LAB-RESULT-W01-W12.json"),
        serde_json::to_vec_pretty(&completed).expect("serialize completed twelve-seed result"),
    )
    .expect("persist completed twelve-seed result");
}
