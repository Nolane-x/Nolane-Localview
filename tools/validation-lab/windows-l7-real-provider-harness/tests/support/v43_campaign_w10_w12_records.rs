...
use super::{
    baseline_cases, campaign_support as support, follow_on_cases, geometry_cases,
    verified_input_cases, verified_input_w11_w12_cases,
};
...
pub async fn execute_records(
    ...
    vec![
        ...
        geometry_cases::run_w10(edge_seed_digest, environment_digest, support::PLATFORM_PROFILE, support::COMPARISON_PROFILE, 110).await,
        verified_input_w11_w12_cases::run_w11(edge_seed_digest, environment_digest, support::PLATFORM_PROFILE, support::COMPARISON_PROFILE, 111).await,
        verified_input_w11_w12_cases::run_w12(edge_seed_digest, environment_digest, support::PLATFORM_PROFILE, support::COMPARISON_PROFILE, 112).await,
    ]
}

pub fn validate_records(records: &[RealProviderLabRecord]) {
    assert_eq!(records.len(), 12, "exact physical campaign must contain twelve records");
    ...
    assert_eq!(
        ...
        support::REQUIRED_CASES
            ...
            .collect::<BTreeSet<_>>(),
        "physical mixed-DPI campaign must bind the exact W01-W12 set"
    );
    for record in records {
        assert_eq!(
            record.result_evidence,
            Some(ResultEvidence::RealProviderIntegrationPass),
            "every exact W01-W12 record must independently pass before campaign closure"
        );
