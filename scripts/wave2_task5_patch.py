...
Path("crates/control/src/visual_full_page.rs").write_text(visual)

lib_path = Path("crates/control/src/lib.rs")
lib = lib_path.read_text()
if "mod visual_full_page;" in lib:
    raise SystemExit("visual_full_page module already exists")
lib = lib.replace("mod visual_diff;\n", "mod visual_diff;\nmod visual_full_page;\n", 1)
needle = "        .merge(visual_diff::router(state.clone()))\n"
if lib.count(needle) != 1:
    raise SystemExit("visual_diff router merge marker mismatch")
lib = lib.replace(
    needle,
    needle + "        .merge(visual_full_page::router(state.clone()))\n",
    1,
)
