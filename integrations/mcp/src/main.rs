...
async fn main() -> Result<()> {
    ...
    for line in stdin.lock().lines() {
        ...
        let req: RpcRequest = match serde_json::from_str(&line) {
            Ok(v) => v,
            Err(e) => {
                writeln!(stdout, "{}", serde_json::to_string(&RpcResponse {
                    jsonrpc: "2.0",
                    id: None,
                    result: None,
...
async fn handle(req: RpcRequest) -> RpcResponse {
    ...
    match result {
        ...
        Err(e) => RpcResponse { jsonrpc: "2.0", id, result: None, error: Some(json!({"code": -32000, "message": e.to_string()})) },
    }
}
...
async fn call_tool(params: &Value) -> Result<Value> {
    ...
    let response = match name {
        ...
        "session.inspect" => {
            let id = args.get("id").and_then(Value::as_str).context("missing id")?;
    ...
    if !status.is_success() { return Err(anyhow::anyhow!("LocalView control returned {status}")); }
    let content = if status == reqwest::StatusCode::NO_CONTENT { json!({"ok": true}) } else { response.json::<Value>().await? };
