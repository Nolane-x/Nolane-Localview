...
Branch: `design/v43-l7-windows-real-provider-closure`

Base: `feat/v43-validation-lab-l7-fake-provider-adapter@d957d60c7ffb6af14f699ae1499352d8dafbd788`
...
Historical repository labels are not rewritten. In particular, merged PR/commit names remain immutable provenance even where a human-facing layer label drifted. New V4.3 artifacts MUST prefer the semantic campaign name and, when a numeric layer is written, use the specification meaning above for provider campaigns.

The implementation plan for this design should add a small test-only semantic registry/assertion around provider campaign construction so that a fake-provider adapter cannot be preregistered as `L7` and a real-provider run cannot be preregistered as `L6`. It must not alter the serialized `CampaignLayer::{L0..L9}` wire representation or rewrite old result artifacts.
...
- historical commit/PR names remain unchanged provenance;
...
- rewrite historical Lab artifacts or PR names.

## 21. Expected implementation sequence after design approval

After this design is approved, the implementation plan should order work as follows:
