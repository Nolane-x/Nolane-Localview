...
Stacked base: `feat/wave5-svelte-live-ownership@57e3e654aa629273cbb55dc7e6a6210f8236a334`

Branch: `feat/wave5-vue-live-ownership`
...
Already landed or stacked below this branch:
...
2. the exact element resolves to a bounded `.vue` component file;
3. explicit `data-component-source` outranks Vue introspection;
4. no secret prop/setup-state value appears in serialized semantic evidence;
5. plain DOM does not fabricate Vue ownership;
6. accessor-backed `__vueParentComponent`, `type` and `__file` are never invoked;
7. absolute/traversal/encoded/non-`.vue` file identities fail closed;
8. the adapter does not read `instance.parent`;
9. no line/column is fabricated.
