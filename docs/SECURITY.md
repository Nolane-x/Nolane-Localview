...
LocalView has two separate consequential authority families and does not convert one into the other. Windows UIA keeps its provider-element V4.3 authority. Managed WebViews use a stable-ref authority for `click`, `focus`, `type_text`, `key` and `scroll`: planning requires a newly completed semantic snapshot containing the exact interactive ref; confirmation is process-local, one-shot and short-lived; payload-bearing actions bind private process-local payloads to durable HMAC commitments; dispatch is fenced to the exact current managed provider/target incarnation and requires a journal-minted one-shot executor permit; executor completion is durably linearized before a fresh post-dispatch semantic observation is admitted; and canonical postconditions are committed as durable `ActionPostconditionReceipt` evidence before a verified outcome is exposed. Restart never restores confirmation, payload or dispatch authority. Missing/stale lineage, unavailable fresh evidence, unsupported contracts, journal failure and capacity exhaustion fail closed. CLI/MCP expose only the two-phase `plan -> confirm -> status` surface; legacy one-step DOM mutation commands and mutation use of the observe-only `/actions` route remain unavailable.
