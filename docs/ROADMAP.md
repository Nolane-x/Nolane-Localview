...
- extend exact CSS source ownership beyond bounded declaration/cascade URL-path evidence when a truthful source-map/compiler authority exists;
...
- bounded Vue live ownership is landed and reconciled with coordinate-independent `ComponentOwnership`. The runtime reads exact-element `__vueParentComponent.type.__file` only through own-data descriptors, retains no Vue props/state/context/setup payload, and backend authority canonicalizes genuine absolute plugin-vue compiler paths under the exact session project root before retention.
- bounded CSS declaration trace and author-cascade authority are live. Matching author declarations are privacy-bounded, inactive media/supports and disabled sheets are excluded, unsafe/cross-authority stylesheets fail closed, and a conservative shorthand-safe property subset can carry a proven author-cascade winner without claiming browser-final provenance or CSS source coordinates.
- the trusted source-edit validation loop is live: human-approved exact source mutation → existing HMR-aware settle → exact fresh semantic/source/postimage revalidation → one redacted current native acquisition → bounded changed-region planning → durable affected-region/viewport fallback evidence → deterministic Verify receipt. Local visual changes persist only selected redacted regions; unchanged verification creates no new Visual artifact.

Remaining integration:

- issue / direct point selection → stable element → component/source resolution. Existing source-open/Ask/Fix/Verify paths are live once a stable `ElementRef` is known, but the human point-select transport is not yet complete.
- exact CSS source-line/column ownership beyond same-origin stylesheet URL-path hints and bounded cascade evidence.
