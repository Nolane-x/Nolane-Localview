...
- Whole-cycle budget authority is now connected for the supported semantic path. `localview-planner` evaluates cumulative `spent + next` against the original contract with saturating arithmetic and scores against remaining budget. Authenticated `POST /v1/sessions/{id}/perception/cycle` owns cumulative state inside one request, is hard-bounded to four steps, repeatedly re-plans from retained evidence, carries cumulative text/image/Chromium reservations, replaces forecast latency with measured elapsed wall-clock time at execution/completion boundaries and re-evaluates using only the planner-owned escalation reason. Caller-supplied `spent`, serialized plans and escalation reasons are rejected. The existing fresh-snapshot bridge action retains its own bounded timeout and is not force-cancelled mid-action in this slice.
...
- an explicit bridge cancellation protocol before any future hard mid-action deadline cancellation is attempted; current semantic actions remain bounded by their existing timeout and the cycle rechecks elapsed latency at boundaries;
...
**Done when:** one button edit normally costs an evidence-backed crop + delta instead of a full-page screenshot, and every visual artifact can be traced to a session/revision/viewport/target. Native viewport acquisition, all three hosted rendered-pixel proofs, artifact/evidence registration, fail-closed fresh-snapshot settling, live freeze/restore, pre-persistence private-region redaction, bounded CSS-region execution, evidence-backed progressive semantic targeting, baseline-driven changed-region scheduling, token-aware visual packet selection, planner-owned four-dimensional Perception Budget authority, live semantic perception execution, retained semantic feedback and single-request whole-cycle semantic budget accounting are now present. Dedicated visual/Chromium perception executors, explicit mid-action cancellation authority, Runtime Resource Governor enforcement, deeper framework/source ownership, guarded stitching, responsive execution and the capture → diff → deterministic verification loop remain.
