...
| Stable capture policy | Partial | Fresh-snapshot DOM/font/image readiness plus HMR/DOM/layout/network quiet-window evaluation is wired through an authenticated five-second fail-closed desktop gate before native viewport capture; animation/transition freeze, private-selector masking and true network in-flight accounting remain |
...
| Network analysis | Partial | fetch/XHR metadata and failures are captured without response bodies and securely drained; metadata quiet periods participate in capture settling, while true in-flight accounting, mock/delay and causal correlation remain |
...
| Local permission/security model | Implemented | Bearer token, loopback control plane, caller/session ownership checks, navigation guard and redaction policy; native capture is dashboard-only, resolves only exact session-owned managed surfaces, fails closed on settle timeout and revalidates the live WebView route immediately before acquisition |
...
| Native screenshot adapter | Partial | Real WebView2/WKWebView/WebKitGTK viewport adapters, bounded desktop artifact persistence, fresh stable-settle gating and Visual evidence metadata path exist with cross-platform compile/test contracts; hosted GUI smoke proving real rendered pixels on all three platforms remains the completion gate |
