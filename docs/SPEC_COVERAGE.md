...
| Stable capture policy | Partial | Exact fresh-snapshot DOM/font/image readiness plus live DOM/layout and fetch/XHR-completion quiet-window evaluation is wired through an authenticated five-second fail-closed desktop gate before native viewport capture. The evaluator also supports a 300 ms HMR gate when an HMR observer signal is present, but framework-specific live HMR signal production is still pending. Animation/transition freeze, private-selector masking and true network in-flight accounting remain |
...
| Local permission/security model | Implemented | Bearer token, loopback control plane, caller/session ownership checks, navigation guard and redaction policy; native capture is dashboard-only, resolves only exact session-owned managed surfaces, fails closed on settle timeout, anchors fresh-snapshot presence to daemon evaluation time rather than the page clock, and revalidates the live WebView route immediately before acquisition |
