...
| Native screenshot adapter | Partial | Real WebView2/WKWebView/WebKitGTK viewport adapters, bounded desktop artifact persistence, fresh stable-settle gating, per-session freeze + private geometry → native capture → exact-token restore → in-memory redaction → persist ordering and Visual evidence metadata path exist. Linux/WebKitGTK and macOS/WKWebView now each have dedicated hosted GUI smoke proof that renders deterministic loopback HTML through a real WebView, captures via the same platform snapshot helper used by production, fully decodes the PNG and validates a known rendered center pixel. The WKWebView proof also exposed and fixed a real production ImageIO encoding bug by materializing the snapshot through TIFF → NSBitmapImageRep → PNG. Equivalent real rendered-pixel proof for Windows WebView2 is the remaining completion gate |
