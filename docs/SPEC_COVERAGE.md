...
| Native screenshot adapter | Partial | Real WebView2/WKWebView/WebKitGTK viewport adapters, bounded desktop artifact persistence, fresh stable-settle gating, per-session freeze + private geometry → native capture → exact-token restore → in-memory redaction → persist ordering and Visual evidence metadata path exist. Linux/WebKitGTK additionally has a dedicated hosted Xvfb GUI smoke that renders deterministic HTML through a real WebView, captures via the production-shared snapshot helper, decodes the PNG and validates a known rendered pixel. Equivalent real rendered-pixel proof for Windows WebView2 and macOS WKWebView remains the completion gate |
