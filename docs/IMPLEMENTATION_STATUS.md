...
8. For a region target or changed-region scheduling pass, verify that the captured frame's CSS viewport still matches the live CSS viewport reported by the freeze receipt. Resize/drift fails closed after restoration and before any artifact write.
9. Revalidate the bounded mask geometry and redact the captured viewport PNG **in memory** using `localview-visual`.
10. For an explicit region target, crop the already-redacted viewport PNG using bounded CSS-to-native scaling, then decode the result again to verify the produced pixel dimensions.
11. For changed-region scheduling, decode the redacted viewport once, resolve a compatible route/viewport/scale/pixel baseline, run the bounded tile/coalescing planner, and choose `unchanged`, bounded `regions`, or `viewport` fallback. The first compatible-frame miss emits one viewport `baseline_reset`; an unchanged pass emits no new visual artifact.
12. For a changed-region plan, crop every selected region from the same decoded redacted frame and register region-scoped evidence. The baseline is committed only after the complete selected evidence emission succeeds; evidence failure leaves the prior baseline authoritative.
13. Persist processed frames and register viewport or dedicated `/evidence/visual-region` metadata only after restore acknowledgement, target validation and complete private-mask application succeed.
...
The viewport **Native screenshot adapter is Implemented**: all three production platform backends have live rendered-pixel acquisition evidence in dedicated hosted GUI gates, in addition to compile/contracts coverage. **Bounded CSS-region execution and baseline-driven changed-region scheduling are also live** above that acquisition path: real native viewport pixels are restored, private-redacted, decoded once, compared against a bounded compatible baseline, reduced to unchanged/region/viewport plans, cropped where appropriate, persisted and registered as Visual evidence. This does **not** make progressive capture complete: automatic element/component/section ownership resolution, token-aware packet selection and the complete capture → diff → deterministic verification loop remain Partial capabilities.

## Next vertical slices

The strongest remaining capture/perception gates are:

1. automatic element/component/section target resolution wired from semantic/runtime ownership into the bounded region path;
2. true network in-flight accounting plus framework-specific live HMR signals;
3. capture → visual region diff → deterministic verification as one end-to-end loop, including token-aware visual packet selection;
