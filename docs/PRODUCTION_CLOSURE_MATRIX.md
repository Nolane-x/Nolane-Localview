...
| Wave 9 post-Apply orchestration | Durable preflight authority plus fresh post-Apply evidence produces a scope-explicit current-target/current-route verification receipt without widening to whole-app proof | Closed for bounded V1 | R10 durable recovery + R13 live hard-contract/safe-mutation catalog + R17 bounded receipt, fresh snapshot provenance and global-handoff fence |
| Whole-impact Autonomous Verified verdict | Application-wide autonomous Verified requires complete hard-contract evidence, mutation closure, known affected-state denominator/revalidation universe, resource/cleanup proof and proven side-effect containment | Post-V1 breadth | V1 does not advertise this verdict; the global receipt remains fail-closed Inconclusive until every whole-impact proof obligation exists |
...
| Native workspace default promotion | Native child WebView may become the default only after composition/focus/crash/DPI policy closure | Post-V1 breadth | V1 deliberately ships the conservative iframe workspace; native child support remains feature-gated and W10 remains required before any mixed-DPI promotion claim |
...
| Update check/channel | User-triggered check uses a compile-time pinned HTTPS manifest URL, rejects redirects/cross-origin artifacts/oversized or malformed manifests, and never grants install authority | Closed | R16 exact-head UI/security/runtime authority; build remains explicitly unconfigured when no channel URL is compiled |
...
| Documentation truth | README/security/status/coverage and production-release docs match the bounded V1 claim; broader research remains explicitly separated | Closed | R18 executable production-truth gate plus final stale-claim audit |
