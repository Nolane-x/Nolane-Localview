...
use localview_desktop::workspace_surface::{
    WorkspaceBounds, bridge_surface_label_allowed, validate_workspace_bounds, workspace_label,
};
...
fn workspace_bounds_reject_invalid_or_unbounded_geometry() {
    ...
    for invalid in [
        ...
        assert!(
            validate_workspace_bounds(invalid).is_err(),
            "invalid bounds were accepted: {invalid:?}"
        );
    }
}
