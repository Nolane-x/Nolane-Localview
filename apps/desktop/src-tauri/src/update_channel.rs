...
fn validate_manifest(
    ...
    let matches = manifest
        ...
        .collect::<Vec<_>>();
    if matches.len() != 1 {
        return Err("update manifest must contain exactly one artifact for this target".into());
    }
...
mod tests {
    ...
    #[test]
    fn exactly_one_current_target_artifact_is_required() {
        let manifest_url = parse_channel_url("https://updates.example.test/manifest.json").unwrap();
        let mut value = manifest("0.3.0", "https://updates.example.test/localview.bin");
        value.artifacts.push(value.artifacts[0].clone());
        assert!(validate_manifest(value, &manifest_url, "0.2.0", "linux", "x86_64").is_err());
    }
